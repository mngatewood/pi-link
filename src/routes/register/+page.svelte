<script>
    ...
    $: if(emailInput && showEmailError) { emailInput.focus() }
    ...
    <div class="w-full">
        <form  method="post" action="?/register" class="mb-8" use:enhance>
            ...
            <div class="form-item">
                ...
                <br>
                <input bind:value={email} class:fieldError={form?.message} id="email" data-testid="register-email"type="email" name="email" required />
            </div>
            ...
            <div class="form-item">
                ...
                <div class="visibility-container">
                    {#if !showPassword}
                        <input bind:value={password} class:fieldError={passMatchError || passLengthError} type='password' id="password" data-testid="register-password" name="password" required />
                    ...
                    {:else}
                        <input bind:value={password} class:fieldError={passMatchError || passLengthError} type='text' id="password" data-testid="register-confirm-pw" name="password" required />
            ...
            <div class="form-item">
                ...
                <div class="visibility-container">
                    {#if !showConfirm}
                        <input bind:value={confirm} class:fieldError={passMatchError || passLengthError} type='password' id="passwordConfirm" name="passwordConfirm" required />
                        <button class="btn-visibility" type="button" tabindex="-1" on:click={() => showConfirm = true}>
                            <img class="mag-glass" src={ VisibleIcon } alt="Magnifying Glass Icon" />
                        </button>
                    {:else}
                        <input bind:value={confirm} class:fieldError={passMatchError || passLengthError} type='text'  id="passwordConfirm" name="passwordConfirm" required />
