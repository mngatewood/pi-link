	...
	if (!locals.pb.authStore.isValid) {
		...
		try {
			...
			const game = await locals.pb.collection('games').getOne(gameId, {
				expand: 'players',
				fields:
					'id, code, host, status, players, round, stage, playerRoles, playerOrder, clue, votes, votingCompleted, results, expand'
			});
...
export const actions = {
	...
	finishVoting: async ({ request, locals }) => {
		...
		if (!formData.isInformant) {
			...
			const conspiratorId = Object.keys(playerRoles).find(
				(key) => playerRoles[key] == 'Conspirator'
			);
			const detectiveIds = Object.keys(playerRoles).filter(
				(key) => playerRoles[key] == 'Detective'
			);
