<script>
	...
	const informantId = Object.keys(data.game.playerRoles).find(
		(key) => data.game.playerRoles[key] == 'Informant'
	);
				...
				{#each candidates as player}
					<label class="radio-label form-item round-header-card player-card">
						<input
							...
							name="candidates"
							value={player.id}
							on:click={() => (disableVoteSubmit = false)}
						/>
		...
		<form method="post" action="?/finishVoting" class="w-full mt-8" use:enhance>
			<div class="form-item">
				...
				<button
					...
					id="continue-results"
					class="href-button button-fit-content button-submit">Proceed to next stage.</button
				>
			</div>
		</form>
	{/if}
