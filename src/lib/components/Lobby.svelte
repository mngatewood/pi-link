...
<div class="container lobby-container">
	<div class="container lobby-header-container">
		...
		{#if isHost}
			<h4>
				Your invitation code is {data.game.code.toString().padStart(4, '0')}.<br />Click 'Start
				Game' once all players have joined.
			</h4>
		{:else}
		...
		{#each game.expand.players as player}
			<div class="player {myCard(player.id)}">
				...
				<form
					...
					method="post"
					id="remove-player"
					style="display:{removeVisibility(player.id)}"
					use:enhance
				>
	...
	<div class="container buttons-container">
		{#if isHost && disableStart}
			<div class="text-center mb-4 alert">
				<small>At least 4 players are required to start the game.</small>
			</div>
		{/if}
